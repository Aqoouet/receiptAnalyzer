...
# Автоматический скрипт для освобождения портов, указанных в config.yaml
# Использование: ./free_ports_auto.sh [config_file]
...
# Функция для автоматического освобождения порта
free_port_auto() {
    local port=$1
    ...
    echo "   🗑️  Автоматически завершаем процессы..."
